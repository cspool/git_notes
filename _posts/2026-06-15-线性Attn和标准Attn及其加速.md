...
$$\log P(X_1, X_2, \dots, X_N) = \sum_{i=1}^{N} \log P(X_i \mid X_{<i})$$

在这个公式中：
* **目标项**：每一个步长中的独立随机变量 $X_i$。
* **条件项**：当前步之前的历史上下文集合 $X_{<i} = \{X_1, \dots, X_{i-1}\}$。

### 1.2 标准 Attention 的非参数化特征对齐
大模型在隐藏层中，需要为每一项条件概率 $\log P(X_i \mid X_{<i})$ 构建一个完美的概率特征表征向量 $o_i$。标准 Attention 机制将其定义为一种**非参数化的条件显式查表（Instance-based Retrieval）**：
...
设 $t=1$ 时：
* $k_1 = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$, $v_1 = \begin{pmatrix} 5 \\ 6 \end{pmatrix}$
* 组装状态：$S_1 = k_1 v_1^T = \begin{pmatrix} 1 \\ 0 \end{pmatrix} \begin{pmatrix} 5 & 6 \end{pmatrix} = \begin{pmatrix} 5 & 6 \\ 0 & 0 \end{pmatrix}$

当 $t=2$ 时，再次迎来相同的输入 $k_2 = k_1 = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$, $v_2 = v_1 = \begin{pmatrix} 5 \\ 6 \end{pmatrix}$。
根据原生线性映射规则，我们从左边注入转置后的键向量 $k_2^T$ 对算子空间进行正交性测试：

$$\text{Prediction} = k_2^T S_1 = \begin{pmatrix} 1 & 0 \end{pmatrix} \begin{pmatrix} 5 & 6 \\ 0 & 0 \end{pmatrix} = \begin{pmatrix} 5 & 6 \end{pmatrix} = v_2^T$$

利用纯数学结合律展开该过程：
$$k_2^T S_1 = k_2^T (k_1 v_1^T) = (k_2^T k_1) v_1^T = 1 \cdot v_1^T = v_1^T$$

计算其原生的自回归条件残差 $e_2$：
$$e_2 = v_2^T - k_2^T S_1 = \begin{pmatrix} 5 & 6 \end{pmatrix} - \begin{pmatrix} 5 & 6 \end{pmatrix} = \begin{pmatrix} 0 & 0 \end{pmatrix}$$
