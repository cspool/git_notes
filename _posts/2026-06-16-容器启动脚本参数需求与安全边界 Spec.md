---
layout: post
title: "容器启动脚本参数需求与安全边界 Spec——用于Agent生成"
date: 2026-06-16 23:30:00 +0800
categories: notes
---
...
- `${HOST_HOME}/.claude/commands`
- `${HOST_HOME}/.claude/agents`
- `${HOST_HOME}/.claude/hooks`
- `${HOST_HOME}/.claude/plugins`
- `${HOST_HOME}/.claude/skills`
- `${HOST_HOME}/.codex/auth.json`
- `${HOST_HOME}/.codex/config.toml`
- `${HOST_HOME}/.codex/plugins`
- `${HOST_HOME}/.codex/rules`
- `${HOST_HOME}/.codex/skills`

### 4.2 规则

1. 不挂载整个 `$HOME`，只挂载明确需要的文件或目录。
2. 认证文件和配置文件默认应倾向 `ro`；只有工具确实需要刷新 token、写 settings 或更新插件时，才允许对特定文件/目录 `rw`。
3. skills 如果只是运行，应使用 `ro`；如果要在容器里编辑 skill，才使用 `rw`，并确认 host 也接受这些改动。
4. skill symlink target 需要解析后挂载真实目标。目标如果落在 package/cache 类目录下，仍然必须 `ro,rshared`。
5. 不要把 secret、token、SSH key、Git credential 等宽泛地 bind 进容器；需要哪个工具就挂载哪个最小路径。
